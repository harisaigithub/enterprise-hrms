...
const ADMIN_USER = "lewis hamilton";
...
function AddBusinessUnitModal({ isOpen, onClose, company, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim()) return;
    setSaving(true);
    const bu = { id: `bu-${Date.now()}`, name: name.trim(), companyId: company.id, status: "Active" };
...
function AddDepartmentModal({ isOpen, onClose, businessUnits, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    const dept = { id: `d-${Date.now()}`, name: name.trim(), businessUnitId, status: "Active" };
    const res = await addDepartment(dept);
    setSaving(false);
    onSaved(res.data);
    onClose();
    setName(""); setBusinessUnitId("");
  };
...
function AddLocationModal({ isOpen, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim()) return;
    setSaving(true);
    const loc = { id: `l-${Date.now()}`, name: name.trim(), city: city.trim(), country: country.trim(), status: "Active", employeeCount: 0 };
...
function LocationsTab({ locations, onAdded, onUpdated }) {
  ...
  const handleDeactivate = async (id) => {
    ...
    if (res.data?.error) {
      setErrors((p) => ({ ...p, [id]: res.data.error }));
      return;
    }
    setErrors((p) => ({ ...p, [id]: null }));
    onUpdated(res.data.location);
  };
...
function AddCostCenterModal({ isOpen, onClose, departments, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    const cc = { id: `cc-${Date.now()}`, code: code.trim(), name: name.trim(), departmentIds, status: "Active" };
    const res = await addCostCenter(cc);
    setSaving(false);
    onSaved(res.data);
    onClose();
    setCode(""); setName(""); setDepartmentIds([]);
  };
...
function DesignationsGradesTab({ designations, grades, onDesignationAdded, onGradeAdded }) {
  ...
  const handleAddDesignation = async (e) => {
    ...
    const d = { id: `ds-${Date.now()}`, title: newDesignation.trim(), status: "Active" };
  ...
  const handleAddGrade = async (e) => {
    ...
    const g = { id: `g-${Date.now()}`, code: newGrade.code.trim(), name: newGrade.name.trim(), order: grades.length + 1, status: "Active" };
    const res = await addGrade(g);
    onGradeAdded(res.data);
    setNewGrade({ code: "", name: "" });
  };
...
function ReportingStructureTab({ roster, departments, auditLog, onManagerUpdated, onBulkReassigned }) {
  ...
  const handleReassign = async (employeeId, newManagerId) => {
    const res = await updateReportingManager(employeeId, newManagerId, ADMIN_USER);
    if (res.data?.error) {
      setErrors((p) => ({ ...p, [employeeId]: res.data.error }));
      return;
    }
    setErrors((p) => ({ ...p, [employeeId]: null }));
    onManagerUpdated(res.data.employee);
  };
...
export default function OrgManagement() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      getCompany(), getBusinessUnits(), getDepartments(), getLocations(),
      getCostCenters(), getDesignations(), getGrades(), getRoster(), getAuditLog(),
    ]).then(([c, bu, d, l, cc, ds, g, r, al]) => {
      setCompany(c.data);
